...
class EventBus:
    ...
    def on_event(self, event_type: Union[Event, str], *, weight: int = 1) -> Optional[Callable]:
        def decorator(func: SubScriberCallback):
            self.subscribe(event_type, func, weight=weight)
            logger.debug(f"{func.__name__} has subscribed to {event_type}, weight={weight}")
    ...
    def on_event_filter(self, event_type: Union[Event, str], *, weight: int = 1) -> Optional[Callable]:
        def decorator(func: FilterCallback):
            self.add_filter(event_type, func, weight=weight)
            logger.debug(f"Event filter {func.__name__} has been added to event {event_type}, weight={weight}")
