...
class McsmInfoMySQLManager(McsmInfoDatabaseManager):
    def __init__(self, database_instance: DatabaseAdapter):
        McsmInfoDatabaseManager.__init__(self, database_instance, DataEngineType.MYSQL)
        if database_instance.get_database_type() != DataEngineType.MYSQL:
            raise ValueError("Database type is not MYSQL")
